...
class Dataset(dict):
    ...
    def __getattr__(self, __name: str) -> Any:
        if __name.startswith(("X_", "y_")):
            ...
            if split_name in self.splits.keys():
                return self.splits[split_name][0] if __name.startswith("X_") else self.splits[split_name][1]
    ...
    def create_from_pipeline(
        ...
        return cls(data=data, data_splitter=data_splitter,target_column=target_column, name=name)
    ...
    def create_from_splits(
        ...
        for split_name, (X, y) in splits.items():
            assert "y" not in X.columns, f"Split {split_name} already has a 'y' column, please drop or rename"
            Xs.append(X.assign(y=y))
        fullX = pd.concat(Xs, ignore_index=True)

        dataset = cls(data=fullx, data_splitter=None, target_column = "y", cache = cache, name = name)
